var OrdersPage = (function () {
  ...
  function _render(q) {
    ...
    if(!orders.length){tbody.innerHTML='<tr><td colspan="8" class="empty-state"><span class="material-symbols-outlined">receipt_long</span>Chưa có đơn hàng nào</td></tr>';return;}
    tbody.innerHTML = orders.map(function(o){
      return '<tr><td><strong>'+o.so_ct+'</strong></td><td>'+o.ngay_ct+'</td><td>'+o.chi_nhanh+'</td>'+
        ...
        '<td style="font-weight:700">'+(o.total_qty||0)+' SP</td>'+
        '<td style="font-weight:700;color:var(--accent)">'+Utils.formatMoney(o.total_money||0)+'</td>'+
        '<td style="color:var(--muted)">'+(o.ghi_chu||'—')+'</td>'+
        '<td style="display:flex;gap:6px">'+
          '<button class="btn btn-ghost btn-sm" onclick="OrdersPage.view(\''+o.id+'\')">Chi tiết</button>'+
