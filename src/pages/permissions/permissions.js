...
var PermissionsPage = (function () {
  ...
  function render(containerElement) {
    ...
    return Router.fetchTemplate('src/pages/permissions/permissions.html')
      .then(function (html) {
        try {
          $container.innerHTML = html;

          // CSS nhỏ cho tree table và role tabs
          var styleId = 'permissions-page-styles';
          if (!document.getElementById(styleId)) {
            ...
            style.innerHTML = `
              ...
              .tree-toggle { font-size: 20px; transition: transform 0.2s; color: var(--text-secondary); width: 20px; text-align: center; }
              .tree-toggle.open { transform: rotate(90deg); }
              .tree-toggle.empty { opacity: 0; pointer-events: none; }
              .tree-icon { font-size: 20px; color: var(--primary); }
              .tree-row[data-hidden="true"] { display: none; }

              .role-tab { padding: 12px 16px; border-radius: 8px; cursor: pointer; transition: all 0.2s; margin-bottom: 4px; display: flex; align-items: center; gap: 8px; font-weight: 500; color: var(--text); }
              .role-tab:hover { background: rgba(148, 163, 184, 0.1); }
              .role-tab.active { background: var(--primary-light); color: var(--primary); }

              #perm-ctx-menu { position: fixed; z-index: 9999; background: var(--surface, #fff); border: 1px solid var(--border, #e2e8f0); border-radius: 10px; box-shadow: var(--shadow-lg); padding: 4px 0; min-width: 240px; max-height: 80vh; overflow-y: auto; display: none; }
              #perm-ctx-menu .ctx-item { padding: 8px 14px; cursor: pointer; font-size: 13px; display: flex; align-items: center; gap: 8px; color: var(--text, #333); transition: background 0.15s; }
              #perm-ctx-menu .ctx-item:hover { background: var(--primary-light); color: var(--primary); }
  ...
  function _fetchGroups() {
    ...
    if (container) {
      container.innerHTML = '<div style="padding: 16px; text-align: center; color: var(--text-secondary);">Đang tải dữ liệu...</div>';
    }
    ...
    PermissionsService.getGroups()
      .then(function (records) {
        if (records.length > 0) {
          ...
          if (container) container.innerHTML = '<div style="padding: 16px; text-align: center; color: var(--danger);">Lỗi tải dữ liệu</div>';
        }
      })
      .catch(function (err) {
        console.error(err);
        if (typeof Alert !== 'undefined') Alert.error('Lỗi', 'Lỗi kết nối máy chủ');
        if (container) container.innerHTML = '<div style="padding: 16px; text-align: center; color: var(--danger);">Lỗi kết nối</div>';
      });
  }
  ...
  function _renderRoleTabs() {
    ...
    groups.forEach(function (g, idx) {
      ...
      div.innerHTML = `${UIIcon.renderHtml('group')} ${g.name || g.UserGroupID}`;
  ...
  function _renderTreeTableForGroup(group) {
    ...
    if (roleNameEl) roleNameEl.innerText = group.name || group.UserGroupID;

    var tbody = $container.querySelector('#permission-tree-table tbody');
    tbody.innerHTML = '<tr><td colspan="12" class="text-center" style="padding: 16px;">Đang tải cấu trúc quyền...</td></tr>';

    PermissionsService.getFullMenusByGroup(group.id || group.UserGroupID)
      .then(function (records) {
        _buildTreeTableFromApi(group, records);
      })
      .catch(function (err) {
        console.error(err);
        tbody.innerHTML = '<tr><td colspan="12" class="text-center text-danger">Lỗi kết nối khi tải quyền</td></tr>';
      });
  }
  ...
  function _buildTreeTableFromApi(group, records) {
    ...
    if (!records || records.length === 0) {
      tbody.innerHTML = '<tr><td colspan="12" class="text-center" style="padding:24px;color:var(--text-secondary);">Chưa có dữ liệu phân quyền cho nhóm này</td></tr>';
      return;
    }
  ...
  function _appendRow(tbody, data) {
    ...
    var toggleHtml = data.isFolder
      ? UIIcon.renderHtml('chevron_right', '', `tree-toggle ${data.expanded ? 'open' : ''}`)
      : UIIcon.renderHtml('chevron_right', '', 'tree-toggle empty');

    tdTree.innerHTML = `
      <div class="tree-cell" style="padding-left: ${paddingLeft}px; padding-top: 8px; padding-bottom: 8px;">
        ${toggleHtml}
        ${UIIcon.renderHtml(data.icon, '', 'tree-icon')}
        <span class="tree-label" style="font-weight: ${data.isFolder ? '700' : '400'};">${data.label}</span>
      </div>
    `;
  ...
  function _syncPermissions() {
    ...
    if (btn) {
      btn.disabled = true;
      btn.innerHTML = `${UIIcon.renderHtml('sync', 'font-size: 16px; margin-right: 4px; animation: spin 1s linear infinite;')} Đang đồng bộ...`;
    }
